...
export default async function Page() {
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    const weekProducts = allProducts.filter((product: { createdAt: Date }) => {
  ...
  const totalValue = allProducts.reduce((sum: number, product: { price: any; quantity: any }) => sum + Number(product.price) * Number(product.quantity), 0)

  const highStockProduct = allProducts.filter((p: { quantity: any }) => Number(p.quantity) > 5).length

  const lowStockProduct = allProducts.filter((p: { quantity: any }) => Number(p.quantity) <= 5 && Number(p.quantity) >= 1).length

  const outOfStockProduct = allProducts.filter((p: { quantity: any }) => Number(p.quantity) === 0).length
